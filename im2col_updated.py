...
'''
# Im2Col: new version (staticmethod)
class Im2Col(Function):
    @staticmethod
    def forward(ctx, input, kernel_size, stride, padding):
        ...
        return im2col_batch(input, ctx.kernel_size, ctx.stride, ctx.padding)
    ...
    def backward(ctx, grad_output):
        if not grad_output.is_contiguous():
            grad_output = grad_output.contiguous()
        assert(grad_output.is_contiguous())
        return col2im_batch(grad_output, ctx.kernel_size, ctx.strice, ctx.padding, ctx.input_size)
# Col2Im: old version
'''
...
class Col2Im(Function):
    @staticmethod
    def forward(ctx, input, kernel_size, stride, padding):#, input_size):
        ctx.kernel_size = kernel_size
        ctx.stride = stride
        ctx.padding = padding
        #ctx.input_size = input_size
        assert(input.is_contiguous())
        return col2im_batch(input, ctx.kernel_size, ctx.stride, ctx.padding)#, ctx.input_size)
